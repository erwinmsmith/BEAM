...
class Graph(ABC):
    ...
    def __init__(self, 
                ...
                bayesian: bool = False, # 新增贝叶斯参数
                use_mcmc: bool = False  # 是否使用MCMC采样
                ):
        ...
        self.dec_1=False
        self.skip_nodes = []
        self.bayesian = bayesian # 使用贝叶斯分布的标志
        self.use_mcmc = use_mcmc  # 是否使用MCMC采样
        ...
        if self.bayesian:
            ...
            # 初始化边的均值和方差
            self.edge_means = torch.zeros(len(self.potential_spatial_edges))
            self.edge_vars = torch.ones(len(self.potential_spatial_edges))
            self.temporal_edge_means = torch.zeros(len(self.potential_temporal_edges))  
            self.temporal_edge_vars = torch.ones(len(self.potential_temporal_edges))
    ...
    def construct_spatial_connection(self, temperature: float = 1.0, threshold: float = None,): # temperature must >= 1.0
        ...
        for potential_connection, edge_logit, edge_mask in zip(self.potential_spatial_edges, self.spatial_logits, self.spatial_masks):
            ...
            if not self.check_cycle(in_node, {out_node}):
                ...
                if self.bayesian:
                    # 使用高斯过程进行推断
                    edge_mean = self.edge_means[self.potential_spatial_edges.index(potential_connection)]
                    edge_var = self.edge_vars[self.potential_spatial_edges.index(potential_connection)]
                    edge_prob = torch.sigmoid(edge_mean / temperature)
                else:
    ...
    def construct_spatial_connection_diff(self, round:int = 0, temperature: float = 1.0, threshold: float = None,): # temperature must >= 1.0
        ...
        if self.use_mcmc:
            # 定义贝叶斯模型和MCMC采样器
            def model(potential_connections, edge_logits, edge_masks):
                edge_means = pyro.sample("edge_means", Normal(torch.zeros_like(edge_logits), torch.ones_like(edge_logits)))
                for i, (connection, logit, mask) in enumerate(zip(potential_connections, edge_logits, edge_masks)):
                    if mask == 0:
                        continue
            ...
            mcmc = MCMC(nuts_kernel, num_samples=50, warmup_steps=5)
            
            # 运行MCMC采样
            mcmc.run(self.potential_spatial_edges, self.spatial_logits[round], self.spatial_masks[round])
            ...
            # 使用采样结果来决定边的连接状态
            for i, (potential_connection, edge_logit, edge_mask) in enumerate(zip(self.potential_spatial_edges, self.spatial_logits[round], self.spatial_masks[round])):
                out_node:Node = self.find_node(potential_connection[0])
                in_node:Node = self.find_node(potential_connection[1])
                ...
                if not self.check_cycle(in_node, {out_node}):
                    ...
                    if i >= len(edge_means_samples):
                        continue  # 跳过超出范围的索引
                    edge_mean = edge_means_samples[i].mean()
                    edge_prob = torch.sigmoid(edge_mean / temperature)
    ...
    def construct_temporal_connection(self, round:int = 0, temperature: float = 1.0, threshold: float = None,):  # temperature must >= 1.0
        ...
        for potential_connection, edge_logit, edge_mask in zip(self.potential_temporal_edges, self.temporal_logits, self.temporal_masks):
            ...
            if self.bayesian:
                # 使用高斯过程进行推断
                edge_mean = self.temporal_edge_means[self.potential_temporal_edges.index(potential_connection)]
                edge_var = self.temporal_edge_vars[self.potential_temporal_edges.index(potential_connection)]
                edge_prob = torch.sigmoid(edge_mean / temperature)
            else:
                edge_prob = torch.sigmoid(edge_logit / temperature)
    ...
    def construct_temporal_connection_diff(self, round:int = 0, temperature: float = 1.0, threshold: float = None,):  # temperature must >= 1.0
        ...
        if self.use_mcmc:
            ...
            # 运行MCMC采样
            mcmc.run(self.potential_temporal_edges, self.temporal_logits[round-1], self.temporal_masks[round-1])
            ...
            # 使用采样结果来决定边的连接状态
            for i, (potential_connection, edge_logit, edge_mask) in enumerate(zip(self.potential_temporal_edges, self.temporal_logits[round-1], self.temporal_masks[round-1])):
                out_node:Node = self.find_node(potential_connection[0])
                in_node:Node = self.find_node(potential_connection[1])
                if edge_mask == 0.0:
                    continue
                ...
                if i >= len(edge_means_samples):  # 检查索引是否超出范围
                    continue
    ...
    async def arun(self, input: Dict[str,str], 
        ...
        for round in range(num_rounds):
            ...
            if round <= 5 and skip:
                ...
                # selected_index = 1
                for i in range(len(loss_f_list)):
                    if i==selected_index:
